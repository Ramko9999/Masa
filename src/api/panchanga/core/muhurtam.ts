...
function getRahuKalam(sunrise: number, sunset: number, date: number): MuhurtamInterval {
...
export function getAbhijitMuhurtam(sunrise: number, sunset: number): MuhurtamInterval {
    const daylightDuration = sunset - sunrise;
...
export function getYamaGandam(sunrise: number, sunset: number, date: number): MuhurtamInterval {
    const dayOfWeek = new Date(date).getDay();
    const daylightDuration = sunset - sunrise;
    const partDuration = daylightDuration / 8;
...
export function getGulikaKalam(sunrise: number, sunset: number, date: number): MuhurtamInterval {
    const dayOfWeek = new Date(date).getDay();
    const daylightDuration = sunset - sunrise;
    const partDuration = daylightDuration / 8;
...
export function getVarjyams(day: number, nakshatra: NakshatraInterval[]): MuhurtamInterval[] {
...
export function compute(sunrise: number, sunset: number, date: number, nakshatra: NakshatraInterval[]): MuhurtamInterval[] {
    const muhurtams = [getRahuKalam(sunrise, sunset, date), getAbhijitMuhurtam(sunrise, sunset), getYamaGandam(sunrise, sunset, date), getGulikaKalam(sunrise, sunset, date), ...getVarjyams(date, nakshatra)].sort((a, b) => a.startTime - b.startTime);
