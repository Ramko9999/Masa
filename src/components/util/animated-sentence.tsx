...
const animatedSentenceStyles = StyleSheet.create({
    container: {
        ...StyleUtils.flexRow(),
        flexWrap: 'wrap',
    },
...
function AnimatedSentenceWord({
    ...
    const wordProgress = useDerivedValue(() => {
        ...
        const wordAnimationProgress = (elapsedAnimationDuration - shouldStartAt) / wordDuration;
    ...
    return (
        <Animated.Text style={[animatedSentenceStyles.word, textStyle, animatedStyle]}>
            {word}
        </Animated.Text>
    );
}
...
export function getTotalAnimationDuration(text: string, wordDuration: number, stagger: number) {
...
export function AnimatedSentence({
    ...
    const totalDuration = getTotalAnimationDuration(content, wordDuration, stagger);
    ...
    useEffect(() => {
        if (startAnimation) {
            progress.value = withDelay(
                initialDelay,
                withTiming(1, { duration: totalDuration, easing: Easing.linear },
                    (done) => {
                        if (done) {
