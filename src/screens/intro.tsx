...
function IntroHeader({ onSkip, currentIndex, totalSlides }: IntroHeaderProps) {
    ...
    return (
        <View style={[introHeaderStyles.container, { paddingTop: insets.top, height: height * 0.12 }]}>
            <IntroPagination length={totalSlides} currentIndex={currentIndex} />
            <Pressable onPress={onSkip}>
                <Text neutral style={introHeaderStyles.skipButton}>Skip</Text>
            </Pressable>
        </View>
    );
}
...
function IntroPagination({ length, currentIndex }: IntroPaginationProps) {
    return (
        <View
            style={introPaginationStyles.container}
        >
...
export function Intro({ navigation }: IntroProps) {
    ...
    const handleOnboardingComplete = async () => {
        ...
        const locationPermissionStatus = await LocationApi.getLocationPermissionStatus();
