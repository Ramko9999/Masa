...
export async function getPanchangaDay(
  day: number
): Promise<PanchangaDayData | null> {
  const dayKey = String(truncateToDay(day));
  const dataRaw = await store.get("panchanga_data");
  if (!dataRaw) return null;

  // ExtensionStorage returns a JSON string, so we need to parse it
  const data: Record<string, PanchangaDayData> =
...
export async function getAllPanchangaData(): Promise<Record<
  string,
  PanchangaDayData
> | null> {
  const dataRaw = await store.get("panchanga_data");
